...
extern template class DominatorTreeBase<GlobalBBCFGNode, true>; 
...
class GlobalBBDomTree : public DominatorTreeBase<GlobalBBCFGNode, true> {
private:
		GlobalBBCFG* cfg;

public:
		typedef DominatorTreeBase<GlobalBBCFGNode, true> Base;

		GlobalBBDomTree() : DominatorTreeBase<GlobalBBCFGNode, true> () {}
		explicit GlobalBBDomTree(GlobalBBCFG  &G) : DominatorTreeBase<GlobalBBCFGNode, true>(), cfg(&G) {
				recalculate(G);
		}
		...
		void getDescendants(const BasicBlock* bb, SmallVectorImpl<const BasicBlock*> &result) const {
				...
				// 1. cfg->getCFGNode(bb) to obtain GlobalBBCFGNode for Basic Block bb
				DominatorTreeBase<GlobalBBCFGNode, true>::getDescendants(cfg->getCFGNode(bb), cfgNodeResult);
