     ...
     if (const Instruction *inst = dyn_cast<Instruction>(val)) {
        if (isa<AllocaInst>(inst)) {
            for (llvm::DbgInfoIntrinsic *DII : FindDbgAddrUses(const_cast<Instruction*>(inst))) {
                if (llvm::DbgDeclareInst *DDI = dyn_cast<llvm::DbgDeclareInst>(DII)) {
                        llvm::DIVariable *DIVar = cast<llvm::DIVariable>(DDI->getVariable());
                    rawstr << "ln: " << DIVar->getLine() << " fl: " << DIVar->getFilename();
                    break;
                }
            }
            ...
            llvm::DILocation* Loc = cast<llvm::DILocation>(N);                   // DILocation is in DebugInfo.h
            unsigned Line = Loc->getLine();
            StringRef File = Loc->getFilename();
            //StringRef Dir = Loc.getDirectory();
            rawstr << "ln: " << Line << " fl: " << File;
